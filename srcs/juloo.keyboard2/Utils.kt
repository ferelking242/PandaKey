...
object Utils
{
  ...
  @JvmStatic
  fun show_dialog_on_ime(dialog: AlertDialog, ims: InputMethodService)
  {
    show_dialog_on_ime(dialog, ims.window!!.window!!.decorView.windowToken)
  }
  ...
  fun read_all_utf8(inp: InputStream): String
  {
    ...
    val buff_length = 8000
    val buff = CharArray(buff_length)
    var l: Int
    while (reader.read(buff, 0, buff_length).also { l = it } != -1)
      out.append(buff, 0, l)
    return out.toString()
  }
  ...
  fun read_all_bytes(inp: InputStream): ByteArray
  {
    val out = ByteArrayOutputStream()
    val buff = ByteArray(128000)
    var l: Int
    while (inp.read(buff).also { l = it } != -1)
      out.write(buff, 0, l)
